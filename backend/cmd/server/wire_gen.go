...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	v := service.ProvideOpenAIGatewayTLSFingerprintRouterServices(tlsFingerprintProfileService, tlsFingerprintRouterService)
	...
	channelMonitorService := service.ProvideChannelMonitorService(channelMonitorRepository, secretEncryptor, settingService, configConfig)
	...
	pluginManager := service.ProvidePluginManager(pluginRepository, secretEncryptor, configConfig, pluginHostInfo, pluginKVStore, openAIGatewayService, proxyRepository)
	...
	contentModerationRepository := repository.ProvideContentModerationRepository(db)
	contentModerationHashCache := repository.NewContentModerationHashCache(redisClient)
	contentModerationService := service.ProvideContentModerationService(settingRepository, contentModerationRepository, contentModerationHashCache, groupRepository, userRepository, proxyRepository, apiKeyAuthCacheInvalidator, emailService, accountRepository)
	...
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, grokOAuthHandler, cnProviderHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, tlsFingerprintRouterHandler, pluginHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, promptAdminHandler, paymentHandler, affiliateHandler, complianceHandler, auditLogHandler, upstreamBillingProbeService, ollamaCloudUsageService)
	...
	mediaVideoRepository := repository.NewMediaVideoRepository(db)
	mediaVideoHTTPClient := service.ProvideMediaVideoHTTPClient()
	mediaVideoDownloadLimiter := repository.NewMediaVideoDownloadLimiter(redisClient, settingService)
	mediaVideoService := service.ProvideMediaVideoService(mediaVideoRepository, accountRepository, usageBillingRepository, apiKeyAuthCacheInvalidator, mediaVideoHTTPClient, billingCacheService, mediaVideoDownloadLimiter, settingService)
	mediaVideoHandler := handler.ProvideMediaVideoHandler(mediaVideoService, openAIGatewayHandler)
	...
	ccSwitchDownloadService := service.NewCCSwitchDownloadService(gitHubReleaseClient)
	ccSwitchDownloadHandler := handler.NewCCSwitchDownloadHandler(ccSwitchDownloadService)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	openAIQuotaAutoResetService := service.ProvideOpenAIQuotaAutoResetService(accountRepository, openAIQuotaService, rateLimitService, idempotencyCoordinator, auditLogService, settingService, leaderLockCache)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, channelMonitorV2Handler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, passkeyHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, modelPlazaHandler, asyncImageHandler, mediaVideoHandler, batchImageHandler, ccSwitchDownloadHandler, idempotencyCoordinator, idempotencyCleanupService, openAIQuotaAutoResetService)
