...
func TestContentModerationRequestsDecodeAIThinkingSettings(t *testing.T) {
	...
	t.Run("api key test", func(t *testing.T) {
		...
		require.NoError(t, json.Unmarshal([]byte(`{"ai_thinking_mode":"disabled","ai_reasoning_effort":"high","ai_synchronous_budget_ms":4500,"ai_fast_stage_budget_ms":2200,"ai_fast_input_chars":16000,"ai_fallback_input_chars":6000,"ai_risk_levels_enabled":false,"ai_observe_threshold":0.42}`), &req))
		...
		require.Equal(t, 2200, req.AIFastStageBudgetMS)
