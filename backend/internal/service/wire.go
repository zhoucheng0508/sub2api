...
// ProvidePluginManager keeps account-directory wiring in the dependency graph
// so regenerating Wire preserves the capability-gated plugin host services.
...
func ProvideImageTaskService(store ImageTaskStore, settings *ImageStorageSettingService, cfg *config.Config) *ImageTaskService {
	return NewImageTaskServiceWithResolver(store, settings.Resolver(), defaultImageTaskTTL, defaultImageTaskExecutionTimeout).
		SetMaxActivePerAPIKey(cfg.ImageTask.MaxActivePerAPIKey)
}
...
// ProvideContentModerationService injects the production account repository
// without widening the backwards-compatible constructor signature.
...
func ProvideChannelMonitorService(
	...
	svc.setInternalProbeAuthenticator(internalprobe.NewAuthenticator(cfg.JWT.Secret), settingService)
