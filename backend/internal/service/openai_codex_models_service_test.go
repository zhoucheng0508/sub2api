...
func newCodexModelsOAuthTestService() *OpenAIGatewayService {
	return &OpenAIGatewayService{httpUpstream: &codexModelsHTTPUpstreamStub{
...
func TestFetchCodexModelsManifestAPIKeyCacheBoundsEntriesAndBodySize(t *testing.T) {
	...
	for i := int64(10); i < int64(11+openAIModelsCacheMaxEntries); i++ {
		account := newCodexModelsAPIKeyTestAccount("https://bounded.example")
		account.ID = i
		fetch(account)
	}
	last := newCodexModelsAPIKeyTestAccount("https://bounded.example")
	last.ID = int64(10 + openAIModelsCacheMaxEntries)
	fetch(last)
	if got := calls.Load(); got != int32(6+openAIModelsCacheMaxEntries) {
	...
	first.ID = 10
	fetch(first)
	if got := calls.Load(); got != int32(7+openAIModelsCacheMaxEntries) {
...
func TestFetchCodexModelsManifestOAuthSharedAcrossGroupsWithIndependentFiltering(t *testing.T) {
	...
	s := &OpenAIGatewayService{
		httpUpstream: newCodexModelsOAuthTestService().httpUpstream,
