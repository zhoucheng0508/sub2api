...
func (cfg *ContentModerationConfig) engineProfile(engine string) *ContentModerationEngineConfig {
	...
	if engine == ContentModerationEngineTypeSafe {
		...
		p = ContentModerationEngineConfig{BaseURL: cfg.BaseURL, Model: cfg.Model, ProxyID: cfg.ProxyID, APIKeys: normalizeModerationAPIKeys(append(append([]string(nil), cfg.APIKeys...), cfg.APIKey)), TimeoutMS: cfg.TimeoutMS, RetryCount: cfg.RetryCount, Thresholds: cfg.Thresholds}
	}
...
func (cfg *ContentModerationConfig) effectiveEngine(engine string) *ContentModerationConfig {
	...
	if cfg.AuditProvider == ContentModerationProviderAIChat {
		out.Engine = ContentModerationEngineOpenAI
		return out
	}
...
func (s *ContentModerationService) updateEngineProfile(ctx context.Context, cfg *ContentModerationConfig, engine string, input UpdateContentModerationEngineInput) error {
	...
	check.Engine = engine
	check.AuditProvider = ContentModerationProviderOpenAIModerations
...
func (s *ContentModerationService) engineConfigView(cfg *ContentModerationConfig) *ContentModerationConfigView {
	...
	for _, engine := range []string{ContentModerationEngineOpenAI, ContentModerationEngineTypeSafe} {
		engineCfg := cloneContentModerationConfig(cfg)
		engineCfg.AuditProvider = ContentModerationProviderOpenAIModerations
		view.EngineConfigs[engine] = s.configView(engineCfg.effectiveEngine(engine))
	}
	return view
}
