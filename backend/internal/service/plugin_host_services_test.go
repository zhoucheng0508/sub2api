...
func TestBuildHostServicesGatesDirectoryByCapability(t *testing.T) {
	...
	m := ProvidePluginManager(nil, nil, nil, PluginHostInfo{}, newFakePluginKVStore(), dir)
	...
	require.True(t, ok)
	require.Same(t, dir, srv.directory, "application wiring must expose the account directory to authorized plugins")
