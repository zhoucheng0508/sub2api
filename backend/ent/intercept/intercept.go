...
// The TLSFingerprintRouterFunc type is an adapter to allow the use of ordinary function as a Querier.
type TLSFingerprintRouterFunc func(context.Context, *ent.TLSFingerprintRouterQuery) (ent.Value, error)

// Query calls f(ctx, q).
func (f TLSFingerprintRouterFunc) Query(ctx context.Context, q ent.Query) (ent.Value, error) {
	if q, ok := q.(*ent.TLSFingerprintRouterQuery); ok {
		return f(ctx, q)
	}
	return nil, fmt.Errorf("unexpected query type %T. expect *ent.TLSFingerprintRouterQuery", q)
}

// The TraverseTLSFingerprintRouter type is an adapter to allow the use of ordinary function as Traverser.
type TraverseTLSFingerprintRouter func(context.Context, *ent.TLSFingerprintRouterQuery) error

// Intercept is a dummy implementation of Intercept that returns the next Querier in the pipeline.
func (f TraverseTLSFingerprintRouter) Intercept(next ent.Querier) ent.Querier {
	return next
}

// Traverse calls f(ctx, q).
func (f TraverseTLSFingerprintRouter) Traverse(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.TLSFingerprintRouterQuery); ok {
		return f(ctx, q)
	}
	return fmt.Errorf("unexpected query type %T. expect *ent.TLSFingerprintRouterQuery", q)
}
...
func NewQuery(q ent.Query) (Query, error) {
	...
	case *ent.TLSFingerprintRouterQuery:
		return &query[*ent.TLSFingerprintRouterQuery, predicate.TLSFingerprintRouter, tlsfingerprintrouter.OrderOption]{typ: ent.TypeTLSFingerprintRouter, tq: q}, nil
