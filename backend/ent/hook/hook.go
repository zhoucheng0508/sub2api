...
// The TLSFingerprintRouterFunc type is an adapter to allow the use of ordinary
// function as TLSFingerprintRouter mutator.
type TLSFingerprintRouterFunc func(context.Context, *ent.TLSFingerprintRouterMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f TLSFingerprintRouterFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.TLSFingerprintRouterMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.TLSFingerprintRouterMutation", m)
}
