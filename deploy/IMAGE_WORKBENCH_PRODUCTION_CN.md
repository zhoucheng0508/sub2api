...
- Vote 线路始终向上游发送同宽高比 1K、`quality=low` 源图；2K、4K 和自定义目标尺寸由 Sub2API 在上传 R2 前使用 Lanczos 重采样；
- CORS 预检允许 `X-Sub2api-Image-Output-Size` 和 `X-Sub2api-Image-Resize-Filter`，但这两个内部控制头不得转发到上游；
- 分别验证 `1024x1024`、`1824x1024`、`2048x2048`、`2048x1152`、`1152x2048`、`3840x2160`、`2160x3840` 和一个合法的 16 倍数自定义尺寸；
- 下载 R2 对象并读取实际宽高，必须与用户目标尺寸完全一致；1K 源尺寸与目标尺寸相同时不得重复编码；
- 验证 1、2、3、10 张设置。当前同一 API Key 仍是单活动任务，因此多张图片必须顺序提交，不能因并发提交出现预期外 429；
- 非 16 倍数、单边超过 3840、像素数越界、宽高比超过 3:1 或非 `lanczos` 算法必须在创建任务前返回 400；
- Lanczos 只增加像素和改善缩放质量，不得在 UI 或运营文案中表述为原生 4K 或 AI 补充细节。
