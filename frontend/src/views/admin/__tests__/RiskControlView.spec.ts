...
describe('admin RiskControlView', () => {
  ...
  it('keeps TypeSafe drafts separate while switching to AI chat and back', async () => {
    const wrapper = mount(RiskControlView, { global: { stubs: { AppLayout: AppLayoutStub, BaseDialog: BaseDialogStub, Icon: true, Select: true, Toggle: true, Pagination: true, ModelWhitelistSelector: ModelWhitelistSelectorStub, ProxySelector: true } } })
    await flushPromises()
    await findButtonByText(wrapper, 'admin.riskControl.openSettings').trigger('click')
    wrapper.findComponent('[data-test="audit-engine-select"]').vm.$emit('update:modelValue', 'typesafe')
    await flushPromises()
    await wrapper.get('[data-test="audit-base-url"]').setValue('https://typesafe-draft.example')
    await wrapper.get('textarea[autocomplete="new-password"]').setValue('typesafe-draft-key')
    const provider = wrapper.findComponent({ name: 'AuditProviderSelector' })
    provider.vm.$emit('update:modelValue', 'ai_chat')
    await flushPromises()
    expect(wrapper.find('[data-test="audit-engine-select"]').exists()).toBe(false)
    await wrapper.get('[data-test="audit-base-url"]').setValue('https://ai-draft.example')
    await wrapper.get('textarea[autocomplete="new-password"]').setValue('ai-draft-key')
    provider.vm.$emit('update:modelValue', 'openai_moderations')
    await flushPromises()
    expect((wrapper.get('[data-test="audit-base-url"]').element as HTMLInputElement).value).toBe('https://typesafe-draft.example')
    expect((wrapper.get('textarea[autocomplete="new-password"]').element as HTMLTextAreaElement).value).toBe('typesafe-draft-key')
    await findButtonByText(wrapper, 'admin.riskControl.saveConfig').trigger('click')
    await flushPromises()
    expect(updateConfig).toHaveBeenCalledWith(expect.objectContaining({
      audit_provider: 'openai_moderations', engine: 'typesafe',
      engine_configs: expect.objectContaining({ typesafe: expect.objectContaining({ base_url: 'https://typesafe-draft.example', api_keys: ['typesafe-draft-key'] }) }),
    }))
    expect(JSON.stringify(updateConfig.mock.calls[0])).not.toContain('ai-draft-key')
