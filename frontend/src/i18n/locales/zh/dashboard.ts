export default {
  ...
  keys: {
    ...
    oneClick: {
      bannerTitle: '一键接入 Codex',
      bannerDescription: '使用当前 API 密钥生成 Codex 配置或导入链接。',
      noEligibleKey: '创建或启用一个 API 密钥后即可接入 Codex。',
      ineligibleHint: '仅已启用且可用的 API 密钥可接入 Codex',
      action: '一键接入',
      dismissBanner: '关闭一键接入提示',
      title: '一键接入',
      currentKey: '当前密钥：{name}',
      currentKeyLabel: '当前密钥',
      currentKeyDescription: '用这个密钥生成导入链接和本地配置。',
      selectKey: '选择 API 密钥',
      manageKeys: '管理 API Key',
      later: '稍后再说',
      securityHint: '完整密钥仅在您主动导入、复制或下载脚本时写入配置。',
      guide: '新手引导',
      ccswitch: '方式二：已安装 CC Switch',
      script: '方式三：一键脚本',
      guideTitle: '配置 Codex',
      guideClientTitle: '配置 {app}',
      installCodexAppTitle: '安装 Codex App',
      installCodexAppDescription: '下载并安装适用于 {os} 的官方 Codex App。',
      installCodexCliTitle: '安装 Codex CLI',
      installCodexCliDescription: 'Linux 暂无官方桌面应用，请按官方指引安装 Codex CLI。',
      downloadCodexWindows: '下载 Windows 版 Codex App',
      downloadCodexMacos: '下载 macOS 版 Codex App',
      openCodexLinuxGuide: '查看 Linux 版 Codex CLI 指引',
      officialAddressHint: '将打开官方地址。',
      ccSwitchDirectDownloadHint: '由本站接口解析并直接下载，无需打开官方 Releases 页面。',
      installCcSwitchTitle: '安装 CC Switch',
      installCcSwitchDescription: '下载并安装 CC Switch，用于写入 Codex 的 API 配置。',
      installCcSwitchDescriptionFor: '下载并安装 CC Switch，用于写入 {app} 的 API 配置。',
      downloadCcSwitch: '下载 CC Switch',
      resolvingCcSwitch: '正在查找下载...',
      architecture: '系统架构',
      ccSwitchVersion: 'CC Switch 版本',
      latestVersion: '最新版本',
      loadingVersions: '正在加载版本…',
      versionSourceHint: '版本由本站接口解析',
      ccSwitchDownloadFailed: '无法启动自动下载。',
      openCcSwitchReleases: '打开官方 Releases',
      importCodexTitle: '接入 API 到 Codex App',
      importCodexDescription: '打开 CC Switch，并将当前 API 密钥导入 Codex 配置。',
      selectClient: '选择要接入的客户端',
      selectClientDescription: '选择 CC Switch 中要写入当前 API 配置的应用。',
      sharedProvider: '共用 Claude provider',
      manualOnly: '需在应用内添加',
      appUnavailable: '此应用暂不支持 provider 深链接导入。',
      piManualReason: 'Pi provider 需在 CC Switch 的 Pi provider 页面中手动添加。',
      clientDefaultModel: '由客户端选择',
      installClientTitle: '安装 {app}',
      installClientDescription: '先安装 {app}，再通过 CC Switch 写入 API 配置。',
      openClientGuide: '查看安装页面',
      importClientTitle: '接入 API 到 {app}',
      importClientDescription: '打开 CC Switch，并将当前 API 密钥导入 {app} 配置。',
      openCcSwitchFor: '打开 CC Switch 并导入 {app}',
      ccswitchTitle: '通过 CC Switch 导入',
      ccswitchDescription: '将当前服务作为 Codex 提供商导入已安装的 CC Switch。',
      ccswitchDescriptionFor: '将当前服务作为 {app} 提供商导入已安装的 CC Switch。',
      client: '客户端',
      model: '模型',
      openCcSwitch: '打开 CC Switch 并导入 Codex',
      scriptTitle: '下载一键配置脚本',
      cnOaiTitle: '国模 OAI 一键配置',
      stepOne: '第一步',
      stepTwo: '第二步',
      stepThree: '第三步',
      cnOaiConfigureTitle: '配置国模 OAI 并接入 Codex',
      cnOaiInstallFirst: '1. 下载并安装 CC Switch',
      cnOaiDescription: '适用于 Windows 10/11。自动安装模型目录、开放图片输入和推理强度选项，并完成 CC Switch 配置，无需管理员权限。选项已开放不代表上游支持所有档位。',
      cnOaiPrerequisite: '先安装并启动一次 CC Switch 和 Codex；关闭代理接管、保存编辑，再从托盘菜单完全退出 CC Switch，最后运行配置脚本。脚本不会强制关闭应用。',
      cnOaiAutomaticModel: '留空自动选择可用模型',
      cnOaiDownload: '下载双击配置文件',
      cnOaiRun: '安装好并完全退出 CC Switch 后，双击下载的 .cmd 文件。完成后打开 CC Switch 并重启 Codex。脚本包含当前密钥，请勿转发；失败原因会显示在窗口中。',
      cnOaiGenerateFailed: '无法生成配置文件，请检查接入地址和所选密钥。',
      scriptDescription: '脚本会先备份现有 Codex 配置，并在备份目录生成恢复脚本。',
      selectOs: '选择操作系统',
      previewComment: 'Sub2API Codex 一键配置脚本',
      backupPreview: '现有配置将在写入前备份，可随时恢复。',
      runCommand: '下载后运行',
      copyScript: '复制脚本',
      scriptCopied: '脚本已复制',
      scriptCopyFailed: '复制失败，请改用下载脚本。',
      downloadScript: '下载配置脚本',
      rollbackHint: '运行后会显示备份目录和恢复命令。'
    },
    enable: '启用',
    disable: '禁用',
    nameLabel: '名称',
