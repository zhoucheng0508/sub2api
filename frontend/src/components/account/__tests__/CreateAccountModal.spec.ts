...
describe('CreateAccountModal OpenAI long-context billing', () => {
  ...
  it('submits GPT-5.6 prompt cache mode for OpenAI API Key accounts', async () => {
    const wrapper = mountModal()
    await selectButtonByText(wrapper, 'OpenAI')
    await selectButtonByText(wrapper, 'API Key')
    await wrapper.get('form#create-account-form input[type="text"]').setValue('cached account')
    await wrapper.get('form#create-account-form input[type="password"]').setValue('test-api-key')

    const select = wrapper
      .findAllComponents({ name: 'Select' })
      .find((candidate) => candidate.attributes('data-testid') === 'openai-prompt-cache-mode-select')
    expect(select).toBeDefined()
    select?.vm.$emit('update:modelValue', 'gpt56_explicit')
    await wrapper.vm.$nextTick()
    await wrapper.get('form#create-account-form').trigger('submit.prevent')
    await flushPromises()

    expect(createAccountMock).toHaveBeenCalledTimes(1)
    expect(createAccountMock.mock.calls[0]?.[0]?.extra?.openai_prompt_cache_mode).toBe(
      'gpt56_explicit'
    )
  })
